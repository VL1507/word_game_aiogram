...
def insert_update_table(msg, list_of_countries: list, list_of_russian_cities: list) -> None:
    ...
    with sqlite3.connect("my_games.sqlite") as con:
        ...
        if (msg.from_user.id,) not in list(cur.execute("SELECT user_id FROM users")):
            cur.execute("INSERT INTO users VALUES (?, ?, ?, ?)", (msg.from_user.id, msg.from_user.username, 0, 0))
        else:
            cur.execute("UPDATE users SET play_countries = ?, play_russian_cities = ? WHERE user_id  = ?", (0, 0, msg.from_user.id))
